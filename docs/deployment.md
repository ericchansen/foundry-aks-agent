...
**Status: live AKS request, Path A registration, and attributed native traces
demonstrated.** The evidence below comes from the deployed workload, Foundry SDK,
...
redeployed, and exercised with a new request; its logs no longer show that warning.
...
above. See the
[failed-child-write diagnostics and targeted remediation](../infra/README.md#public-ip-feature-gate-hidden-by-a-nonterminal-aks-operation).

The original empty attempts and node groups were removed; the current
IaC-backed cluster and dedicated foundations remain running. The steps below
reproduce the request and attribution against an approved deployment. Agent
execution remains on AKS; the operator performs only the client request,
registration, deployment, and telemetry queries.
...
[README](../README.md). Never deploy the all-zero sentinel digest or a mutable
